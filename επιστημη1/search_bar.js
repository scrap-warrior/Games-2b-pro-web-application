...
function pop_up_window(){
  window.alert(day+' '+date+'  '+month+' '+year+'\n'+greeting+'!\n'+'   Εδώ θα βρείτε μια τεράστια συλλογή από τα καλύτερα βιντεοπαιχνίδια, για όλες τις κονσόλες και τα γούστα. Ανακαλύψτε νέες κυκλοφορίες, αγαπημένα classics και μοναδικές προσφορές που δεν θέλετε να χάσετε!');}

function SendMail1(){
  var body= document.getElementById('Message').value;
  var subjectline = document.getElementById("Subject").value
  window.location.href = "mailto:nekpar48@gmail.com?subject="+subjectline+"&body="+body;
}

function SendMail2(){
  var body= document.getElementById('Message').value;
  var subjectline = document.getElementById("Subject").value;
  window.location.href = "mailto:g.roditis2@gmail.com?subject="+subjectline+"&body="+body;
}

function SendMail3(){
  var body= document.getElementById('Message').value;
  var subjectline = document.getElementById("Subject").value;
  window.location.href = "mailto:dstathis888@gmail.com?subject="+subjectline+"&body="+body;
}

function my_paragraph1(){
  paragraph = document.getElementById("description");
  paragraph.style.backgroundColor = "rgb(142, 130, 130)"; //επέλεξε το χρώμα πίσω από την παράγραφο.
}

function my_paragraph2(){
  paragraph = document.getElementById("description");
  paragraph.style.backgroundColor = "#a7a1a1"; //επέλεξε το χρώμα πίσω από την παράγραφο.
}

let score = 0; // Αρχικό σκορ
let currentQuestion = 1; //τρέχουσα ερώτηση
let guest_name = "";
...
function start_quiz() {
  ...
  guest_name = document.getElementById("name").value; //το όνομα του παίκτη
...
function answer_question(answer) {
  ...
  if (currentQuestion <= 6) {
    ...
    // Τέλος quiz
    alert("Τέλος του quiz, " + guest_name + "! Το σκορ σου είναι: " + score);
    document.getElementById("questions").style.display = "none";
  }
}

function show_question(questionNumber) {
  // Απόκρυψη των ερωτήσεων
  for (let i = 1; i <= 6; i++) {
